...
class AMP(Optimizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        defaults = dict(
            lr=lr, epsilon=epsilon, inner_lr=inner_lr, inner_iter=inner_iter, **kwargs
        )
    ...
    def step(self, closure=None):
        if closure is None:
            raise ValueError(
                "Adversarial model perturbation requires closure, but it was not provided"
            )
            ...
            for group in self.param_groups:
                for p in group["params"]:
                    if p.grad is not None:
                        ...
                        p.sub_(self.state[p]["dev"]).add_(
                            dev
                        )  # update "theta" with "theta+delta"
