...
def parse_arguments():
    ...
    parser.add_argument(
        "-i", "--in_dir", help="Input directory with source images", required=True
    )
    parser.add_argument(
        "-o", "--out_dir", help="Output directory for resized images", required=True
    )
    parser.add_argument(
        "-s",
        "--size",
    ...
    parser.add_argument(
        "-a",
        "--algorithm",
        help="Algorithm used for resampling: lanczos, nearest,"
    ...
    parser.add_argument(
        "-e",
        "--every_nth",
        help="Use if you don't want to take all classes, "
    ...
    parser.add_argument(
        "-j",
        "--processes",
        help="Number of sub-processes that run different folders " "in the same time ",
        default=1,
        type=int,
    )
...
def resize_img_folder(in_dir, out_dir, alg):
    ...
    if alg_val is None:
        print(
            "Sorry but this algorithm (%s) is not available, use help for more info."
            % alg
        )
        return
    ...
    for filename in os.listdir(in_dir):
        ...
        try:
            ...
            print(f"This File couldn't be read as an image {filepath}")
            with open("log.txt", "a") as f:
                f.write(f"Couldn't resize: {filepath}")
